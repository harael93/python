...
const CourseNavigator = ({ modules = courseData, user, initialProgress }) => {
  ...
  // Helper to initialize completedLessons from backend JSON
  function getCompletedLessonsFromBackend(data) {
    try {
      const arr = JSON.parse(data);
      // Validate shape: array of arrays of booleans
      if (Array.isArray(arr) && arr.every(mod => Array.isArray(mod))) {
        return arr;
      }
    } catch {
    ...
    // fallback: all false
    return modules.map((mod) => mod.lessons.map(() => false));
  }

  const [completedLessons, setCompletedLessons] = useState(() => {
    if (initialProgress != null) {
      return getCompletedLessonsFromBackend(initialProgress);
    }
